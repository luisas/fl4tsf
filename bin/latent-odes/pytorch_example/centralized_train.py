...
# Hyperparameters
# TODO SYNC with federated training
lr = 0.001
epochs = 5
batch_size = 32
...
model = Net()
trainloader, testloader = load_data(partition_id=0, num_partitions=20)

# train
loss_training = train(model, trainloader, epochs, lr=lr, device=device, loss_per_epoch=True)
...
if not os.path.exists("outputs/centralized_outputs"):
    os.makedirs("outputs/centralized_outputs")
