...
def save_client_dataset(client_id, client_data, args):
    ...
    train_set = {
        'data': signals_tensor[train_indices],
        'time_steps': timestamps_tensor[train_indices],
        'amplitude': [],  # Not used for ECG data
        'frequency': []   # Not used for ECG data
    }
    
    test_set = {
        'data': signals_tensor[test_indices],
        'time_steps': timestamps_tensor[test_indices], 
        'amplitude': [],
        'frequency': []
    }
    ...
    # Save metadata with additional information about padding
    metadata_df = pd.DataFrame(client_data['metadata'])
    
    # Add padding information to metadata
    original_lengths = [signal.shape[0] for signal in client_data['signals']]
    metadata_df['original_length'] = original_lengths
    metadata_df['padded_length'] = max_length
    metadata_df['padding_applied'] = [length < max_length for length in original_lengths]
